...
export default class FireExp {
  constructor(wx, wy, wz) {
    ...
    const uniforms = {
      diffuseTexture: {
        value: this.resources.items.fireExpTexture,
      },
      pointMultiplier: {
        value:
...
*/
    ...
    this._colourSpline.AddPoint(0.0, new THREE.Color(0x99a1b7));
    this._colourSpline.AddPoint(1.0, new THREE.Color(0xbdc8cf));
...
*/
  ...
  _AddParticles(timeElapsed) {
    timeElapsed += 0.01 * 0.01;
    if (!this.gdfsghk) {
      this.gdfsghk = 0.0;
    }
    this.gdfsghk += timeElapsed;
    const n = Math.floor(this.gdfsghk * 2.0);
    this.gdfsghk -= n / 2.0;
  ...
  _UpdateParticles(timeElapsed) {
    ...
    for (let p of this._particles) {
      ...
      p.currentSize = 20; // p.size * this._sizeSpline.Get(t) + 20;
